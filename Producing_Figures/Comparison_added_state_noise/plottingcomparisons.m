...
subplot(2,1,1)
hold on
plot(EKFSS.time.t, Measurements(:,1), EKF4.time.hat,EKF4.y.hat(1,:), EKF3.time.hat,EKF3.y.hat(1,:), EKF2.time.hat,EKF2.y.hat(1,:), EKFL.time.hat,EKFL.y.hat(1,:), 'k.')
...
legend('Data','SS4','SS3','SS2','L')
ylabel('Position [m]')
% axis([Time.start,Time.end,-0.15,0.15])
subplot(2,1,2)
hold on
plot(EKFSS.time.t, Measurements(:,2), 'g')
plot(EKFSS.time.hat, EKFSS.x.hat(2,:),'b')
plot(EKF2.time.hat,EKF2.x.hat(2,:),'m')
plot(EKFL.time.hat,EKFL.x.hat(2,:),'r')
